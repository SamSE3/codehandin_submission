...
/**
 * Prints a particular instance of codehandin_submission
 *
 * This shows the checkpoints and tests for a particular
 * codehandin_submission assignment. Should probably use a renderer to show specific. things.
 *
...
$mform->addElement('selectyesno', 'maxbytes', "form elements");

$id = optional_param('id', 0, PARAM_INT); // course_module ID, or
$n = optional_param('n', 0, PARAM_INT); // codehandin_submission instance ID - it should be named as the first character of the module
$show = optional_param("show", 0, PARAM_INT);
$student_id = optional_param('user', 0, PARAM_INT);

if ($id) {
    $cm = get_coursemodule_from_id('codehandin_submission', $id, 0, false, MUST_EXIST);
    $course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $codehandin = $DB->get_record('codehandin_submission', array('id' => $cm->instance), '*', MUST_EXIST);
} elseif ($n) {
    $codehandin = $DB->get_record('codehandin_submission', array('id' => $n), '*', MUST_EXIST);
    $course = $DB->get_record('course', array('id' => $codehandin->course), '*', MUST_EXIST);
    $cm = get_coursemodule_from_instance('codehandin_submission', $codehandin->id, $course->id, false, MUST_EXIST);
} else {
...
add_to_log($course->id, 'codehandin_submission', 'view', "view.php?id={$cm->id}", $codehandin->name, $cm->id);

require_capability('mod/codehandin_submission:view', $context);
...
$submission = $DB->get_record("codehandin_submission", ["id" => $codehandin->id, "userid" => $submission_user_id]);
$subbox = '';
...
if ($teacher && $show) {
    $subs = $DB->get_records("codehandin_submission", ["id" => $codehandin->id]);
    $table = new html_table();
    ...
    foreach ($subs as $s) {
        ...
        $student = $DB->get_record("user", ["userid" => $s->userid]);
...
elseif ($teacher && $student_id) {
    ...
    $sql = "SELECT * FROM {codehandin_checkpoint} WHERE {codehandin_checkpoint}.assign_id = {$cm->instance}";
    if (!$teacher) {
        $sql .= " AND {codehandin_test}.assessment = 0";
    }
    $sql .= " ORDER BY {codehandin_checkpoint}.id ASC"; // change .ordering to .id
    ...
    if (count($checkpoints) == 0) {
        ...
        foreach ($checkpoints as $v) {
            ...
            if (!$tests) {
                $cp .= "There doesn't appear to be any tests for this checkpoint.";
            } else {
