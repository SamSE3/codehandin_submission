 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignfeedback_codehandin_import_zip_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'uploadzip', get_string('confirmuploadzip', 'assignfeedback_codehandin'));
        ...
        foreach ($files as $unzippedfile) {
            ...
            if ($importer->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($importer->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if (!$grade || !$exists) {
                        $updates[] = get_string('feedbackfileadded', 'assignfeedback_codehandin',
                                            array('filename'=>$filename, 'student'=>$userdesc));
                    } else {
                        $updates[] = get_string('feedbackfileupdated', 'assignfeedback_codehandin',
                                            array('filename'=>$filename, 'student'=>$userdesc));
                    }
                }
            }
        ...
        if (count($updates)) {
            ...
            $mform->addElement('html', get_string('nochanges', 'assignfeedback_codehandin'));
        }
