 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignfeedback_codehandin_zip_importer {
    ...
    public function delete_import_files($contextid) {
        ...
        return $fs->delete_area_files($contextid,
                                      ...
                                      assignfeedback_codehandin_IMPORT_FILEAREA,
                                      $USER->id);
    }
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(assignfeedback_codehandin_MAXFILEUNZIPTIME);
     ...
     *
     * @param assign $assignment - The assignment instance
     * @param assign_feedback_codehandin $fileplugin - The file feedback plugin
    ...
    public function import_zip_files($assignment, $fileplugin) {
            ...
            if ($this->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                                                 'assignfeedback_codehandin',
                                                 assignfeedback_codehandin_FILEAREA,
                                                 $grade->id,
                                                 '/',
                                                 $filename)) {
                        ...
                        $newfilerecord->component = 'assignfeedback_codehandin';
                        $newfilerecord->filearea = assignfeedback_codehandin_FILEAREA;
        ...
        $importsummary = new assignfeedback_codehandin_import_summary($assignment->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $assignrenderer = $assignment->get_renderer();
        $renderer = $PAGE->get_renderer('assignfeedback_codehandin');
        ...
        $o .= $assignrenderer->render(new assign_header($assignment->get_instance(),
                                                        ...
                                                        get_string('uploadzipsummary', 'assignfeedback_codehandin')));
